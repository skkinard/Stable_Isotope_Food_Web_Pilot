...
trophic_plot <- function(my_data) {
  my_data %>%
    ...
    geom_point(size=4, shape = 23, aes(fill=site, color=site), 
               position=position_dodge(width = .5)) +
    #geom_point(aes(x=m_group, y = m_group_sig), 
    #           color = 'red', shape = 8, size = 3, show.legend =F) +
    scale_fill_manual(breaks= c('Sub-Humid', 'Mesic','Semi-Arid'),
                      values = rev(my_colors)) +
    scale_color_manual(breaks= c('Sub-Humid', 'Mesic','Semi-Arid'),
