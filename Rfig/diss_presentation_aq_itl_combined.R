...
plot_aq_tl <-  function(x) {
  ...
  myout <- x %>%
    ...
    geom_smooth(method='lm', se=F, color = 'grey70', linetype=2,
                linewidth=.3) +
...
p_aq_itl_i_tro <- d %>% filter(substr(taxa,1,2) == 'I-') %>%
  ...
  ylim(c(-0.5, 3.5)) +
  theme(legend.title = element_blank()) +
  geom_jitter(aes(fill=site, shape=taxa, scolor = site),
              size=5, alpha=.8,
              position = position_jitter(width = 0.5, height = 0.5)) +
  stat_poly_eq(use_label(c("R2", "P")),
               label.y = "top", label.x = "left",
...
p_aq_itl_f_spe +
  ...
  (p_aq_itl_i_tro +
     ...
     theme(legend.position=c(.8,.87),
           legend.box.margin = margin(1,1,1,1)) +
     ylab(element_blank())) +
  ggtitle('Invertebrate Functional Groups')
