...
df <- de %>%
  ...
  right_join(dc %>% 
               ...
               select(site_code, collection_date, month, UID, Argia:Odontomyia) %>%
               replace(is.na(.), 0) %>%
               ungroup()) %>%
  unique() %>%
...
invertebrate.Hellinger <- disttransform(spec, method='hellinger')
Ordination.model2 <- rda(invertebrate.Hellinger ~ . ,data=env2, scaling="species")
...
#----------------------------------------------------------------------------
# Species
spec.envfit <- envfit(plot2, env=invertebrate.Hellinger)
...
# Export
#----------------------------------------------------------------------------
write_csv(RDA_coordinates_out, 'Rout/RDA_coordinates_invertebrate.csv')
write_csv(RDA_axis_out, 'Rout/RDA_axis_invertebrate.csv')
write_csv(RDA_vectors, 'Rout/RDA_vectors_invertebrate.csv')

#----------------------------------------------------------------------------
# End 01_RDA_invertebrate_calc